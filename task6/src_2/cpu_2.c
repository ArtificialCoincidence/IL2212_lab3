...
#define IMAGE_ORIGINAL1     0X102100
#define IMAGE_GRAY1         0X103100
...
int main(void) {
  ...
  while (1) {
    if (IORD_8DIRECT(CPU2_TODO, 0) == 1 && IORD_8DIRECT(CPU2_DONE, 0) == 0) {
      IOWR_8DIRECT(CPU2_TODO, 0, 0);

      if (DEBUG) {
        printf("graySDF1 execute!\n");
      }

      toGrayInt((unsigned char *) IMAGE_ORIGINAL1, (unsigned char *) IMAGE_GRAY1);

      if (DEBUG) {
        printGray((unsigned char *) IMAGE_GRAY1, SIZE_X, SIZE_Y);
      }

      IOWR_8DIRECT(CPU2_DONE, 0, 1);
    }
  }
