...
# fi
REMAKE_BSP=true # force true to add ucosii
...
if [ ! -d ${BSP_PATH}_0 ] || [ "$REMAKE_BSP" = true ]; then
    ...
    echo "***********************************************"
    echo ""
    nios2-bsp ucosii ${BSP_PATH}_0 $CORE_FILE \
	      --cpu-name ${CPU}_0 \
...
for i in `seq 0 $NODES`; do
    ...
    xterm -title "${CPU}_$i" -e "nios2-terminal -i $i > ${CPU}_$i.log" &
done
...
for i in `seq 0 $NODES`; do
    ...
    echo ""
    nios2-elf-size ${APP}_$i.elf > ${APP}_${i}_elf.log
done
