  ...
  while (1) {
    if (IORD_8DIRECT(CPU2_TODO, 0) == 1 && IORD_8DIRECT(CPU2_DONE, 0) == 0) {
      ...
      if (DEBUG) {
        ...
        printP3ToAscii((unsigned char *) IMAGE_SHARE, SIZE_X, SIZE_Y);
        printf("-----------------------------------------------------\n");
      }
        
      resize((unsigned char *) IMAGE_SHARE, (unsigned char *) IMAGE_SHARE);

      if (DEBUG) {
        printP3ToAscii((unsigned char *) IMAGE_SHARE, SIZE_X / 2, SIZE_Y / 2);
      }
